Simulate Sliding Window Go Back N Protocol. 
Program Code- 
#include<iostream.h> 
#include<conio.h> 
#include<math.h> 
...
{ event=frame_arrival; 
cout<<"\n\n\t\t\t**********Sender Side**********"; 
if(frame_to_send==(err+(MAX_SEQ/2))%(MAX_SEQ+1) && err_con==1 && err>=0) //to maintain the timeout period 
{ event=timeout; 
frame_to_send=err; 
err_con=0; 
} 
else if(frame_to_send==MAX_SEQ && frame_to_send!=frame_expected && err_con==1) //to restart from the last nak frame 
...
void main() 
{ int n; 
cout<<"\n\tEnter the size of window(in bits) : "; cin>>n; 
MAX_SEQ=pow(2,n)-1; 
go_back_n ob; 
ob.sender(); 
} 
